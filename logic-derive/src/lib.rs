    ...
    match param {
        GenericParam::Type(ref t) => t.bounds.iter().find_map(|b| {
            if let TypeParamBound::Trait(trait_bound) = b {
                if trait_bound.path.segments.last().map(|s| s.ident.to_string()) == name {
                    return Some(&t.ident);
                }
            }
            None
...
fn find_interner(s: &mut synstructure::Structure) -> (TokenStream, DeriveKind) {
    ...
    if let Some(param) = has_interner(&generic_param0) {
        ...
        s.add_where_predicate(parse_quote! { _I: ::logic_ir::interner::Interner });
        s.add_where_predicate(
            parse_quote! { #param: ::logic_ir::interner::HasInterner<Interner = _I> },
        );
...
fn derive_has_interner(mut s: synstructure::Structure) -> TokenStream {
    ...
    s.bound_impl(
        quote!(::logic_ir::interner::HasInterner),
        quote! {
...
fn derive_super_visit(s: synstructure::Structure) -> TokenStream {
    derive_any_visit(s, parse_quote! { SuperVisit }, parse_quote! { super_visit_with })
}
...
fn derive_any_visit(
    ...
    if kind == DeriveKind::FromHasInterner {
        ...
        s.add_where_predicate(parse_quote! { #param: ::logic_ir::visit::Visit<#interner> });
    }

    s.add_bounds(synstructure::AddBounds::None);
    s.bound_impl(
        quote!(::logic_ir::visit:: #trait_name <#interner>),
        quote! {
            fn #method_name <'i, B>(
                &self,
                ...
                #interner: 'i
            {
                ...
                ::logic_ir::visit::ControlFlow::CONTINUE
            }
        },
    )
}
...
fn derive_zip(mut s: synstructure::Structure) -> TokenStream {
    ...
    let mut body = each_variant_pair(&mut a, &mut b, |v_a, v_b| {
        ...
        for (b_a, b_b) in v_a.bindings().iter().zip(v_b.bindings().iter()) {
            quote!(logic_ir::zip::Zip::zip_with(zipper, #b_a, #b_b)?;).to_tokens(&mut t);
        }
    ...
    quote!((_, _)  => Err(::logic_ir::LogicError::NoSolution)).to_tokens(&mut body);

    s.add_bounds(synstructure::AddBounds::None);
    s.bound_impl(
        quote!(::logic_ir::zip::Zip<#interner>),
        quote! {

            fn zip_with<Z: ::logic_ir::zip::Zipper<#interner>>(
                zipper: &mut Z,
                a: &Self,
                b: &Self,
            ) -> ::logic_ir::LogicResult<()>
                {
                    match (a, b) { #body }
                }
        },
...
fn derive_fold(mut s: synstructure::Structure) -> TokenStream {
    ...
    let result = if kind == DeriveKind::FromHasInterner {
        ...
        s.add_impl_generic(parse_quote! { _U })
            .add_where_predicate(
                parse_quote! { #param: ::logic_ir::fold::Fold<#interner, Result = _U> },
            )
            .add_where_predicate(
                parse_quote! { _U: ::logic_ir::interner::HasInterner<Interner = #interner> },
            );
        quote! { #type_name <_U> }
    } else {
