...
impl<I: Interner> InferCtxt<I> {
    ...
    pub fn unify(&self, t: &Term<I>, u: &Term<I>) -> Result<Term<I>, UnificationError> {
        ...
        match (t, u) {
            ...
            (&TermData::Infer(x), t) | (t, &TermData::Infer(x)) => {
                let term = Term::intern(self.interner, t.clone());
                self.instantiate(x, term.clone());
                Ok(term)
            }
            (TermData::Structure(f, xs), TermData::Structure(g, ys)) if f == g =>
                Ok(Term::intern(self.interner, TermData::Structure(*f, self.unify_iter(xs, ys)?))),
    ...
    fn instantiate(&self, idx: InferenceIdx, t: Term<I>) {
